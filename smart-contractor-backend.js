...
// Fix: RIPEMD160 npm package required for Node.js 17+
require('ripemd160');
...
function generateXPRKeyPair() {
    ...
    const pubChecksum   = new RIPEMD160().update(compPub).digest().slice(0, 4);
    const xprPublicKey  = 'PUB_K1_' + base58Encode(Buffer.concat([compPub, pubChecksum]));
